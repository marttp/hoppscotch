...
pub(crate) fn curl_version_for_url(version: http::Version, url: &str) -> curl::easy::HttpVersion {
    ...
    if version == http::Version::HTTP_2 && url.starts_with("http://") {
...
mod tests {
    ...
    #[test]
    fn uses_prior_knowledge_for_cleartext_http2() {
        let version = curl_version_for_url(http::Version::HTTP_2, "http://localhost:8080");
        assert_eq!(version as isize, HttpVersion::V2PriorKnowledge as isize);
    }

    #[test]
    fn uses_negotiated_http2_for_tls() {
        let version = curl_version_for_url(http::Version::HTTP_2, "https://example.com");
        assert_eq!(version as isize, HttpVersion::V2 as isize);
    }

    #[test]
    fn leaves_other_http_versions_unchanged() {
        let version = curl_version_for_url(http::Version::HTTP_11, "http://localhost:8080");
